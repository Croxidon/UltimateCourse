...
{
	Super::BeginPlay();
	GetWorldTimerManager().SetTimer(SpawnWaitTimer, this, &AItem::AfterSpawn, 0.5f);
...
{
	const FString OverlapEndMessage = OtherActor->GetName() + FString("left bounds.");
	IPickupInterface* PickupInterface = Cast<IPickupInterface>(OtherActor);
	if (PickupInterface)
	{
